...
def generate_pick_merge_random(
    random_file, random_percent, indicator_file, pick_percent, save_file, reverse=True
):
    P = []
        ...
        for i in range(10):
            ...
            for item in idx:
                if item not in exist_idx:
                    iddx.append(item)
            left = int(total_imgs * (random_percent + pick_percent) / 100) - len(
                table[str(random_percent)][str(i)]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random-file", type=str, default="dataseed/COCO_supervision.txt"
    )
    parser.add_argument("--random-percent", type=float, default=10.0)
    parser.add_argument(
        "--indicator_file", type=str, default="results/10random_maxnorm.txt"
    )
    parser.add_argument("--pick_percent", type=float, default=10.0)
    parser.add_argument("--reverse", type=bool, default=True)
    parser.add_argument(
        "--save_file", type=str, default="dataseed/pick_maxnorm10+random10.txt"
    )
