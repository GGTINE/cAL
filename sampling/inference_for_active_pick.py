...
def inference(Trainer, cfg):
    ...
    ensem_ts_model.modelTeacher.roi_heads.box_predictor.register_forward_hook(
        box_predictor_hooker
    )
    ...
    for j, item in enumerate(dataset_dicts):
        ...
        for i in range(len(res[0]["instances"])):
            box_info = {
                "confidence score": np.float(
                    res[0]["instances"].scores.cpu().detach().numpy()[i]
                ),
                "pred class": np.int(
                    res[0]["instances"].pred_classes.cpu().detach().numpy()[i]
                ),
                "pred box": res[0]["instances"]
                .pred_boxes.tensor[i]
                .cpu()
                .detach()
                .numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--static-file", type=str, default="temp/coco/static_by_random10.json"
    )  # Json file of the intermediate process
