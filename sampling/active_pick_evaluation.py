...
def CombineMetrics(
    ...
    for image_path, _ in data.items():
        _final_value = (
            difficult_indicators[image_path] * weights["difficult"]
...
def special_(args):
    ...
    (
        ...
        information_indicators,
        diversity_indicators,
    ) = PreprocessData(file_name=args.static_file)
...
if __name__ == "__main__":
    ...
    )  # Json file of the inferenced datasets info
    parser.add_argument(
        "--indicator-file", type=str, default="results/coco/15random_maxnorm"
