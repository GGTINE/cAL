...
from custom_detectorn2.layers.loss_func import (
    ClassificationLoss,
    RegressionLoss,
    CenterLoss,
)
...
class ActiveTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        self.cls_loss = ClassificationLoss(
            cfg.MODEL.FCOS.LOSS_ALPHA, cfg.MODEL.FCOS.LOSS_GAMMA
        )
        self.reg_loss = RegressionLoss(
            cfg.MODEL.FCOS.LOC_LOSS_TYPE,
            cfg.MODEL.FCOS.KLLOSS_WEIGHT,
            cfg.MODEL.FCOS.REG_MAX,
            cfg.MODEL.FCOS.REG_DISCRETE,
            cfg.SEMISUPNET.TS_BETTER,
            cfg.SEMISUPNET.T_CERT
        )
        self.ctr_loss = CenterLoss()
    ...
    def run_step_full_semisup(self):
        ...
        with autocast(enabled=self.cfg.SOLVER.AMP.ENABLED):
            labeled_instances = self.model(label_data_q)

            labels = labeled_instances.labels.flatten()
            pos_inds = torch.nonzero(labels != self.cfg.MODEL.FCOS.NUM_CLASSES).squeeze(
                1
            )
            ...
            # classification loss
            class_target = torch.zeros_like(labeled_instances.logits_pred)
            class_target[pos_inds, labels[pos_inds]] = 1
            cls_loss = (
                self.cls_loss(labeled_instances.logits_pred, class_target) / num_pos_avg
            )

            # only compute center and regression loss for the foreground classes
            labeled_instances = labeled_instances[pos_inds]

            # regression loss
            reg_loss = self.reg_loss(
                labeled_instances.reg_pred,
                labeled_instances.reg_pred_std,
                labeled_instances.reg_targets,
            )

            # center loss
            ctr_loss = (
                self.ctr_loss(
                    labeled_instances.reg_targets, labeled_instances.ctrness_pred
                )
                / num_pos_avg
            )
            ...
            losses = (cls_loss + reg_loss + ctr_loss) / self.accumulation
        ...
        if self.iter >= self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            # visual_img(unlabel_data_q, branch="unlabeled")
            with autocast(self.cfg.SOLVER.AMP.ENABLED):
                unlabeled_instances = self.model(unlabel_data_q, branch="unlabel")

                un_cls_ins = unlabeled_instances["cls"]
                un_reg_ins = unlabeled_instances["reg"]

                labels = un_cls_ins.labels.flatten()
                pos_inds = torch.nonzero(
                    labels != self.cfg.MODEL.FCOS.NUM_CLASSES
                ).squeeze(1)
                ...
                # classification loss
                class_target = torch.zeros_like(un_cls_ins.logits_pred)
                class_target[pos_inds, labels[pos_inds]] = 1
                cls_loss_pseudo = (
                    self.cls_loss(un_cls_ins.logits_pred, class_target)
                    / num_pos_avg
                )
                cls_loss_pseudo *= self.cfg.SEMISUPNET.UNSUP_LOSS_WEIGHT

                # only compute center and regression loss for the foreground classes
                un_cls_ins = un_cls_ins[pos_inds]

                if num_pos_local > 0:
                    # center loss
                    ctr_loss_pseudo = (
                        self.ctr_loss(
                            un_cls_ins.reg_targets,
                            un_cls_ins.ctrness_pred,
                        )
                        / num_pos_avg
                    )
                    ctr_loss_pseudo *= self.cfg.SEMISUPNET.UNSUP_LOSS_WEIGHT

                    # regression loss
                    labels = un_reg_ins.labels.flatten()
                    pos_inds = torch.nonzero(
                        labels != self.cfg.MODEL.FCOS.NUM_CLASSES
                    ).squeeze(1)
                    un_reg_ins = un_reg_ins[pos_inds]

                    reg_loss_pseudo = self.reg_loss(
                        un_reg_ins.reg_pred,
                        un_reg_ins.reg_pred_std,
                        un_reg_ins.reg_targets,
                        un_reg_ins.boundary_vars,
                    )
                    reg_loss_pseudo *= self.cfg.SEMISUPNET.UNSUP_REG_LOSS_WEIGHT

                else:
                    reg_loss_pseudo = torch.tensor(0).to(device=cls_loss_pseudo.device)
                    ctr_loss_pseudo = torch.tensor(0).to(device=cls_loss_pseudo.device)

                unlabeled_loss_dict = {
                    "loss_cls_pseudo": cls_loss_pseudo,
                    "loss_reg_pseudo": reg_loss_pseudo,
                    "loss_ctr_pseudo": ctr_loss_pseudo,
                }
                loss_dict.update(unlabeled_loss_dict)

                losses += (
                    cls_loss_pseudo + reg_loss_pseudo + ctr_loss_pseudo
                ) / self.accumulation
    ...
    def create_pseudo_label(
        self, unlabel_data, pred_teacher, raw_pred_teacher, nms_method, add_reg
    ):
