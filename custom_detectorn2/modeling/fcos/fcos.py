...
from custom_detectorn2.utils.comm import compute_locations
...
class FCOS(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.fcos_head = FCOSHead(
            cfg, [input_shape[f] for f in cfg.MODEL.FCOS.IN_FEATURES]
        )
    ...
    def forward(
        ...
        Returns:
            결과 (list[BoxList] or dict[Tensor]): 모델로 부터 출력이 return됨
                학습동안, loss를 포함한 dict[Tensor]가 return됨
                테스트동안, loss와 추가로 scores, labels, mask (Mask R-CNN)를 포함한 [BoxList]가 return됨
        """
        features = [features[f] for f in self.cfg.MODEL.FCOS.IN_FEATURES]
        ...
        (
            ...
            ctrness_pred,
            bbox_towers,
        ) = self.fcos_head(features, self.cfg.MODEL.FCOS.YIELD_PROPOSAL)

        if self.training:
            instances = self.fcos_outputs.prepare_instance(
                logits_pred,
                reg_pred,
                ctrness_pred,
                locations,
    ...
    def compute_locations(self, features):
        ...
        for level, feature in enumerate(features):
            ...
            locations_per_level = compute_locations(
                h, w, self.cfg.MODEL.FCOS.FPN_STRIDES[level], feature.device
            )
...
class FCOSHead(nn.Module):
    ...
    def forward(self, x, yield_bbox_towers=False):
        logits = []
        bbox_reg = []
        bbox_reg_std = []
        ctrness = []
