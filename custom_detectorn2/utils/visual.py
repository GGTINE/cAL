    ...
    if branch == "labeled":
        ...
        for x in data:
            ...
            for i in range(len(x["instances_class"])):
                ins = x["instances_class"]
                draw_box(image, ins, i)

            for i in range(len(x["instances_reg"])):
                ins = x["instances_reg"]
                draw_box(image2, ins, i)
...
def draw_box(image, ins, i):
    classes = ins[i].gt_classes.tolist()[0]
    boxes = ins[i].gt_boxes.tensor.tolist()[0]
    certers = ins[i].gt_boxes.get_centers().tolist()[0]
    try:
        conf = ins[i].cls_confid
        text = coco_class[classes + 1] + ": " + str(round(conf.tolist()[0], 3))
    except:
        conf = 1.0
        text = coco_class[classes + 1] + ": " + str(conf)
    cv2.rectangle(
        image,
...
def show_box(image):
    filename = "result_" + time.strftime("%Y%m%d_%H%M%S") + ".png"
    filepath = os.path.join(
        "/mnt/c/Users/TINE/Desktop/CAL(Centerness-Active Learning)/pseudo_result/",
        filename,
    )
    count = 1
    while os.path.exists(filepath):
        filename = (
            "result_" + time.strftime("%Y%m%d_%H%M%S") + "_" + str(count) + ".png"
        )
        filepath = os.path.join(
            "/mnt/c/Users/TINE/Desktop/CAL(Centerness-Active Learning)/pseudo_result/",
            filename,
